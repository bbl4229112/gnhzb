...
<head>
	...
	<link rel="stylesheet" type="text/css"
		...
		<script src="<%=basePath%>js/lca/lca_modulelook.js" type="text/javascript"></script>
	...
	<script type="text/javascript">
		...
		function main(container, outline, toolbar, modelbar, status)
		{
				...
				type=label.substring(0,endindex);
						...
						this.isCellEditable(cell))
					{
						...
						alert(processid);
						alert(moduleid);
						var data= cims201.utils.getData('http://localhost:8080/gnhzb/lca/lcamodule!getprocesscontent.action',{moduleid:moduleid,processid:processid});
						cell.name=data.name;
						cell.stage=data.stage;
						var box=Edo.create(
							{type: 'box',width: '100%',height:'70%',border: [0,0,0,0],padding: [0,0,0,0],layout: 'vertical',
				           	    ...
				           	    children : [{type : 'text',width : 200,id : 'processname',text:cell.name}]
				           	    },
				           	    {	type : 'formitem',label : '流程执行阶段:',labelWidth : 150,labelAlign : 'right',
				           	    children : [{type : 'text',width : 200,id : 'stage',text:cell.stage}]
				           	    }
					    			]
								});
							//bbl
							var toolbar=new gettoolbar(null,null);
							var winprocess=cims201.utils.getWin(400,200,'流程信息',[box,toolbar]);
				...
				var data= cims201.utils.getData('http://localhost:8080/gnhzb/module/module!addmodule.action',{modulename:modulename,modulenote:modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduletype});
				}else if(moduletype=='LCA'){
				for(var i=0;i<cells.length;i++){
				    if(cells[i].nodeid!=null){
					var cell={};
					cell.id=cells[i].getId();
				...
				var data= cims201.utils.getData('http://localhost:8080/gnhzb/lca/lcamodule!addmodule.action',{version:versionid,componentid:cmpid,pdid:pdid,modulename:modulename,modulenote:modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduletype});
	    ...
	    children: [
	     {type: 'button', text: '新增', arrowMode: 'menu',
	        ...
	        {type: 'button', text: '查看模型信息',onclick: function(e){
        		...
        		getmodulebyid(graph,r.moduleid);
        		moduleid=r.moduleid;
		            		
		            		
	        }
	        },
	        {type: 'button', text: '编辑模型信息',onclick: function(e){editor.execute('zoomOut');}}    
	    ]
		});
		...
		var producttreebox = Edo.create({
			...
			children:[
	    		...
	    		children:[
		            ...
		            var componentmodelTree=new getcomponentmodelTree();
		            Edo.get('producttree').addChild(componentmodelTree);
		            Edo.util.Ajax.request({
			        type: 'post',        
			        url: 'http://localhost:8080/gnhzb/lca/lcamodule!getmoduleComponentsList.action',
			        params: {
...
<body onload="main(document.getElementById('graphContainer'),
	...
	<div id="toolbarContainer"
		style="position:absolute;white-space:nowrap;overflow:hidden;top:0px;left:0px;max-width:550px;width:550px;max-height:40px;height:36px;right:0px;padding:5px">
	</div>
    <div id="modelbarContainer"
		style="position:absolute;overflow:hidden;top:0px;left:550px;max-height:40px;height:36px;padding:5px;right:0px">
	</div>
