...
<head>
	...
	<script type="text/javascript">
		...
		level.prototype.addlevelmodule=function(cell){
			this.levelmodules.add(new levelmodule(this,cell));
		}
		...
		function main(container, outline, toolbar, modelbar,sidebar, status)
		{
				...
				{ 
					...
					if(cells.length>0&&!moduleobject.issaved){
						...
						if(moduleobject.moduletype=='PDM'){
							...
							var node = enc.encode(graph.getModel());
							var xmldata = mxUtils.getPrettyXml(node);
							var data= cims201.utils.getData('http://localhost:8080/gnhzb/lca/lcamodule!addmodule.action',{version:moduleobject.versionid,components:moduleobject.components,pdid:moduleobject.productid,modulename:moduleobject.modulename,modulenote:moduleobject.modulenote,cellcollection:cellcollection,xmldata:xmldata,moduletype:moduleobject.moduletype});
			    ...
			    children: [
			     /* {type: 'button', width:60,height:30,text: '新增', arrowMode: 'menu',style:'background:CCFF00',
			       ...
			       {type: 'button', text: '编辑下一层模型',width:70,height:35,style:'border-radius:10px;',onclick: function(e){
			       ...
			       if(cell!=null&&editor.graph.model.getStyle(cell)=='process'){
				          var level=wholelevel.getcurrentlevel();
					       ...
					       for(var i=0;i<modules.length;i++){
						       if(modules[i].levelmoduleobject.cell==cell){
			       ...
			       {type: 'button', text: '返回上一层模型',width:70,height:35,style:'border-radius:10px;',onclick: function(e){
			       			initgraph();
			      		var level3=wholelevel.getcurrentlevel();
				        
			      		var parentlevel=level3.getparentlevel();
			      		  if(rootlevell==parentlevel){
				          alert('是的2')}
			            var modules1=parentlevel.levelmodules;
			            var cells=new Array();
			            alert(modules1.length)
			             for(var i=0;i<modules1.length;i++){
				             var cell=modules1[i].levelmoduleobject.cell;
				             cells.add(cell)
			          /*    editor.graph.addCell(cell); */
			             }
			             editor.graph.addCells(cells);
			             wholelevel.setcurrentlevel(parentlevel);
      		
			       }},
			       {type: 'button', text: '查看该层数据',width:70,height:35,style:'border-radius:10px;',onclick: function(e){
			      alert(wholelevel.getcurrentlevel().levelmodules.length);
			       }},
				...
				//		output.geometry.offset = new mxPoint(-8, -4);
					...
					finally
					{
              		...
              		/* var level=wholelevel.getcurrentlevel();
              		 ...
              		 if(modules.length>0){
					       for(var i=0;i<modules.length;i++){
					       alert(modules[i].levelmoduleobject.cell.getId())
					       }
					       } */
