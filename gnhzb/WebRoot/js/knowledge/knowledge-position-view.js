function createPositionKnowledgeView(id,kData) {
	...
	if(kData=='error') {
		...
		globalid=id;		
		topBar = new createPositionKnowledgeTopBar(kData.titleName,id,kData.borrowFlowId,null,kData.ktype.id,kData.knowledgeSourceFilePath).getTopBar();	
		businessActivity = new createBusinessActivity(kData.titleName,kData.flashFilePath,kData.id,kData.knowledgeSourceFilePath).getKnowledge();
//		commentPanel = new createComment(kData.id,kData.ktype.name).getComment();		

		commentPanel = new createComment(kData.id,kData.ktype.name,null).getComment(); //用户评论
		actionPanel = new createAction(kData.attachments).getAction();           //支撑知识
		
		citationPanel = new createCitations(kData.citationKnowledges).getCitations();  //引证文献			
		versionsPanel = new createVersions(kData.id).getVersions();           //版本管理
		...
		centerPanel = Edo.create({
		    ...
		    layout: 'vertical',    
		    children: [
		               topBar,businessActivity, centerTab, centerContent
		    ]
		});	
		if(null == keepAdd ){
...
//知识展示最顶端的工具条
function createPositionKnowledgeTopBar(myTitle,id,borrowFlowId,borrowFlowNodeId,ktypeId,kSourceFilePath){
...
//创建flash形式的知识展示
function createBusinessActivity(titleName,flashFilePath,kid,knowledgeSourceFilePath){
	var knowledgeModule;
	var myqsBox=null;
	var myWin2 = null;
	var knowledgeModule;
...
//				'&xmlFileId='+kid+'&xmlFileName='+titleName
			src: 'PositionKnowledgeView.jsp?xmlFileId='+kid+'&xmlFileName='+encodeURIComponent(titleName)
//			src: 'businessactivities/BusinessActivities.swf?xmlFileId='+kid+'&xmlFileName='+titleName
		});	
//		alert(titleName);
	}	
			...
			children: [
			           {
			        	   ...
			        	   children:[
			        	             {type: 'radiogroup', id: 'approvalResult'+arr[i], displayField: 'name',checkField: 'checked',
			        	            	 valueField: 'value',data: [
                                                     {name: '审批通过', value: '通过'},
                                                     {name: '审批不通过', value: '未通过'},				        	                                                                                                                                     {name: '审批通过但不允许继续升级', value: '通过但停止升级'}
                                                     ]}
			        	             ]
			           },
			           {
