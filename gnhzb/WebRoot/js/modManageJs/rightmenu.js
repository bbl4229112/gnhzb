...
$.fn.extend({RightMenu: function(id,options){options = $.extend({menuList:[]},options);var menuCount=options.menuList.length;
	...
	return this.each(function(){
		...
		this.oncontextmenu=function(e){
			...
			if('myMenu4Mod'==id){
				//设置div的attribute,如果点击的是div节点直接将 e.target.id的值赋给moduleid，如果是img节点，将其父节点id赋值给moduleid
				if('DIV'==e.target.tagName){
					$("#"+id).attr('moduleid',e.target.id);
					$('#myMenu4Part').hide();
					$("#"+id).hide().css({top:my,left:mx}).show();
				}else if('DIV'==e.target.parentNode.tagName){
					$("#"+id).attr('moduleid',e.target.parentNode.id);
					$('#myMenu4Part').hide();
					$("#"+id).hide().css({top:my,left:mx}).show();
				}
			}else if('myMenu4Part'==id){
				if('DIV'==e.target.tagName){
					$("#"+id).attr('partid',e.target.id);
					$('#myMenu4Mod').hide();
					$("#"+id).hide().css({top:my,left:mx}).show();
				}else if('DIV'==e.target.parentNode.tagName){
					$("#"+id).attr('partid',e.target.parentNode.id);
					$('#myMenu4Mod').hide();
					$("#"+id).hide().css({top:my,left:mx}).show();
				}
			}
