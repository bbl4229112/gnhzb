...
//构造左侧的工具栏
//导航栏管理器
function navManager01(type,filename){
	var centerRegion = Edo.get('centerRegion');
	var myNavPanel = Edo.get('centerPanel');
	
	centerRegion.set('visible',true);
	
	if('user' == type){
		centerRegion.set('title', '用户中心');
		if(myNavPanel){
			myNavPanel.destroy();
		}
		...
		centerRegion.addChild(myNavPanel);
	}else if('knowledge' == type){
		centerRegion.set('title', '知识仓库');
		if(myNavPanel){
			myNavPanel.destroy();
		}
		myNavPanel = Edo.create({
		...
		centerRegion.set('visible',false);
