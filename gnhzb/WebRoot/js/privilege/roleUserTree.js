...
function createRoleUserTree(cn,cp, btNames,btFunctions){
	var currentNode = cn;
	...
	*/
	...
	var treeColumns = [{
		...
		roleTree = new createTree({},treeColumns,treeData,'cascade2',[],[],onTreeSelection,onTreeBodySelection);
		...
		var myColumns1 = [
                	  ...
                	  
                {
                    ...
                    renderer: function(v,r){
                    	...
                    	out_str += '<span class="icon-cims201-delete" onclick="deleteSelectedRole('+r.id+');">';
                    	out_str += '&nbsp';                    	
                    	out_str += '</span>';
                    	return out_str;
                    }           	
    	...
    	myTable1_rt = new createTable(myConfig1,'100%','40%','已选角色',myColumns1,[],[],'', {},false);
    	...
    	myTable2_rt = new createTable(myConfig2,'100%','60%','已选人员',myColumns2,[],[],'', {},false);
    	...
    	var roleUserPanel = Edo.create({
		    ...
		    layout: 'vertical',    
		    children: [
		        myTable1_rt.getTable(), myTable2_rt.getTable()
		    ]
    	});
		...
		*/
		function onTreeBodySelection(sId){
			if(sId){
				var powerset = cims201.utils.getData('empowerment!listUsersOutEmpowerment.action',{nodeId:sId,currentEmpowerment:currentPowerset});
	    		myTable_rt.setTableData(powerset);
			}
		}
