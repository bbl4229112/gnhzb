...
//      added by egeg 2010-5-17 13:34:25 for fill the windows
		private function doubleClickHandler(event:MouseEvent):void {
			
			if(OrDesignerModelLocator.getInstance().getOrchestraDesigner().currentState==null||
				OrDesignerModelLocator.getInstance().getOrchestraDesigner().currentState=="")
			new FigureEditorNavigatorAppEvent(FigureEditorNavigatorAppEvent.FILL_THE_WINDOWS).dispatch();
			
			else
			new FigureEditorNavigatorAppEvent(FigureEditorNavigatorAppEvent.DEFAULT_WINDOWS).dispatch();		
		}
