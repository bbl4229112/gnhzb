package org.act.od.impl.commands
{
	...
	public class CoReceiveMessageChangeSizeCMD extends AODCommand
	{
		public function CoReceiveMessageChangeSizeCMD()
		{
			super();
		}
		override public function execute(event:OrDesignerEvent):void
		{
			...
			var xArray_Move:Array = new Array();
			var yArray_Move:Array = new Array();
			var rxArray_Move:Array = new Array();
			var ryArray_Move:Array = new Array();
			var widthArray_Move:Array = new Array();
			var heightArray_Move:Array = new Array();
			var standardWidthArray_Move:Array = new Array();
			...
			xArray_Move = moveFigureMessage.headers.modify_x.split(",");
			yArray_Move = moveFigureMessage.headers.modify_y.split(",");
			rxArray_Move = moveFigureMessage.headers.modify_rx.split(",");
			ryArray_Move = moveFigureMessage.headers.modify_ry.split(",");
			widthArray_Move = moveFigureMessage.headers.modify_width.split(",");
			heightArray_Move = moveFigureMessage.headers.modify_height.split(",");
			standardWidthArray_Move = moveFigureMessage.headers.modify_standardWidth.split(",");
			...
			for (var i:int=0; i < moveFigureXML.child("Figure").length(); i++)
			{
				...
				{
					for(var j:int = 0;j<xArray_Move.length;j++)
						if (moveFigureXML.Figure[i].@ID == idArray_Move[j])
						{
							moveFigureXML.Figure[i].@x=xArray_Move[j];
							moveFigureXML.Figure[i].@y=yArray_Move[j];
							moveFigureXML.Figure[i].@rx=rxArray_Move[j];
							moveFigureXML.Figure[i].@ry=ryArray_Move[j];
							moveFigureXML.Figure[i].@width=widthArray_Move[j];
							moveFigureXML.Figure[i].@height=heightArray_Move[j];
							moveFigureXML.Figure[i].@standardwidth=standardWidthArray_Move[j];
