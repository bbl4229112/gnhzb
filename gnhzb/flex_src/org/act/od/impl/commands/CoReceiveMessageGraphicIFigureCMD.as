package org.act.od.impl.commands
{
	...
	public class CoReceiveMessageGraphicIFigureCMD extends AODCommand
	{
		public function CoReceiveMessageGraphicIFigureCMD()
		{
		...
		override public function execute(event:OrDesignerEvent):void
		{
			...
			var figurenameArray_Move:Array = new Array();
			...
			figurenameArray_Move = moveFigureMessage.headers.modify_figurename.split(",");
			...
			linkName_Move = moveFigureMessage.headers.modify_nameLink.split(",");
				...
				if(moveFigureXML.Figure[i].@type=="Link")
				{
					for(var k:int = 0;k<IDLinkArray_Move.length;k++)
						if(moveFigureXML.Figure[i].@ID==IDLinkArray_Move[k])
						{
							...
							moveFigureXML.Figure[i].@figureName=linkName_Move[k];
				...
				else
				{
					for(var j:int = 0;j<xArray_Move.length;j++)
						if (moveFigureXML.Figure[i].@ID == idArray_Move[j])
						{
							...
							moveFigureXML.Figure[i].@figureName=figurenameArray_Move[j];
