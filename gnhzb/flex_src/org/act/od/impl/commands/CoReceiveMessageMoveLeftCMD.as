package org.act.od.impl.commands
{
	 ...
	 *modify the original xml for moveMoveLeft Operation
	 * @author Mengsong
	 *
	 */
	public class CoReceiveMessageMoveLeftCMD extends AODCommand
	{
		public function CoReceiveMessageMoveLeftCMD()
		{
			super();
		}
			...
			for (var i:int=0; i < moveFigureXML.child("Figure").length(); i++)
			{
				...
				else
				{
					for(var j:int = 0;j<idArray_Move.length;j++)
						...
						{	
							moveFigureXML.Figure[i].@x=(int(moveFigureXML.Figure[i].@x) - 5);
							moveFigureXML.Figure[i].@rx=(int(moveFigureXML.Figure[i].@rx) - 5);
						}
				}
			}
