package org.act.od.impl.commands
{
	 ...
	 * Cooperation for moving right
	 * @author Mengsong
	 *
	 */
	public class CoReceiveMessageMoveRightCMD extends AODCommand
	{
		public function CoReceiveMessageMoveRightCMD()
		{
			super();
		}
			...
			for (var i:int=0; i < moveFigureXML.child("Figure").length(); i++)
			{
				...
				else
				{
					for(var j:int = 0;j<idArray_Move.length;j++)
						...
						{	
							moveFigureXML.Figure[i].@x=(int(moveFigureXML.Figure[i].@x) + 5).toString();
							moveFigureXML.Figure[i].@rx=(int(moveFigureXML.Figure[i].@rx) + 5).toString();
						}
				}
			}
