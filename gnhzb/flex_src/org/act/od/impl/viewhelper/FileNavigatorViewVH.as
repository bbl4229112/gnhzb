package org.act.od.impl.viewhelper
{
	...
	public class FileNavigatorViewVH extends ViewHelper
	{
		...
		public var fileIDT :String ;
		public var filePathT: String ;
		public var fileNameT: String ;
		public var relativeFigureFileIDT :String;
		public var relativeBpelFileIDT :String;	
		 ...
		 */
		public function get fileNavigatorView() :FileNavigatorView{
		...
		public function itemDeleteIt(event :ContextMenuEvent):void{
			var deleteConfirmWindow :DeleteConfirmWindow= DeleteConfirmWindow(PopUpManager.createPopUp(OrDesignerModelLocator.getInstance().getOrchestraDesigner(), DeleteConfirmWindow,true));
			deleteConfirmWindow.fileName=this.fileNavigatorView.selectedItem.@name;
			PopUpManager.centerPopUp(deleteConfirmWindow);
			deleteConfirmWindow.addEventListener(CloseEvent.CLOSE, itemDeleteHandler);
		...
		private function fileOpen():void{
			...
			fileIDT = this.fileNavigatorView.selectedItem.@ID;
			filePathT = this.getSelectedItemPath();
			fileNameT = this.fileNavigatorView.selectedItem.@name;
		...
		public function fileRename(event :ContextMenuEvent):void{
			var reName :SaveFileWindownew = SaveFileWindownew(PopUpManager.createPopUp(OrDesignerModelLocator.getInstance().getOrchestraDesigner(),SaveFileWindownew,true));
			reName.setRenameTitle("重命名");
		...
		private function reNameResult(event :CloseEvent):void{
			var newName:String=SaveFileWindownew(event.currentTarget).getText();
			if(newName != ""){
