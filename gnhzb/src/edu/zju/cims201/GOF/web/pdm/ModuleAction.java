...
public class ModuleAction extends CrudActionSupport<Material> implements ServletResponseAware, ServletRequestAware {
...
/*	public String addlevelmodule() throws Exception{
		System.out.println(moduleuuid);
		PdmModule parent= moduleService.getparentmodule(moduleuuid.trim(),1);
		String uuid = UUID.randomUUID().toString();
		...
		m.setCreatedate(new Date());
		m.setModuleUUID(moduleuuid.trim());
		...
		Set<ProcessTemplate> processTemplates=new HashSet<ProcessTemplate>();
		 ...
		 System.out.println(n);
		   for(int i=0;i<n;i++){
			   ...
			   String processtype=String.valueOf(h.get("processtype"));
			   ...
			   }else if(processtype=="taskprocess"){
				   String processid=String.valueOf(h.get("id"));
				   p.setProcessid(Integer.valueOf(processid));
				   Node node=new Node();
				   p.setName(String.valueOf(h.get("processname")));
	...
	/*public String addmodule() throws Exception{
		...
		String xmlFileName=path+uuid+"//"+uuid+".xml";
		saveXml(xmlFileName);
		BaseModule m=null;
		System.out.println(moduletype);
		if(moduletype.equals("PDM")){
			m=new PdmModule();
			m.setName(modulename);
			m.setNote(modulenote);
			m.setCreatedate(new Date());
	...
	public void getsubmodule() throws IOException{
		...
		PdmModule m=(PdmModule)moduleService.getPdmModulebyparentandprocess(parentlevelid,processid,superparentmoduleid);
		if(m==null){
		}else{
	...
	public String getmoduleprocesscontent() throws IOException{
		 PdmProcessTemplate pt=(PdmProcessTemplate)moduleService.getprocess(processid, Integer.valueOf(moduleid));
...
/*	public void getProcessTaskTreeNode() throws IOException{
		ProcessTemplate p=moduleService.getprocess(processid, Integer.valueOf(moduleid));
	    Map<String, Object> map = new HashMap<String, Object>();
	    map.put("id", p.getTasktreenode().getId());
	    map.put("name", p.getTasktreenode().getNodeName());
...
/*	
	...
	public String getfunctionsbyprocessid() throws IOException{
		...
		Node node=p.getNode();
		Set<Function> functions=node.getFunctions();
		List<Function> functionlist=new ArrayList<Function>();
		 Iterator it=functions.iterator();
			...
			JSONWriter writer = new JSONWriter();
	        String functionstring=writer.write(functionlist);
	        response.getWriter().println(functionstring);
		s
