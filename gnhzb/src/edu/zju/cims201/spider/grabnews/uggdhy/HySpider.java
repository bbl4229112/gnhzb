...
import edu.zju.cims201.spider.grabnews.uggdhy.parser.ParserHy;
import edu.zju.cims201.spider.grabnews.uggdhy.rule.HyUrlRule;
 ...
 * 
 */
public class HySpider extends Thread implements ISpiderReportable {
	private static final HySpider m_bzwspider = new HySpider();
	...
	protected static String _hostUrl = "http://www.uggd.com/news/hynews/";
...
//			); 
	...
	protected HyUrlRule rule = new HyUrlRule();
	protected ParserHy parser = new ParserHy();
	private static String spidername = "BzwSpider";

	private static String tablename = "workload";

	private static String urlpattern = "http://www.uggd.com/news/hynews/\\d{4}-\\d{2}-\\d{2}/\\d{5}.html";

	private static String defaulturl = "http://www.uggd.com/news/hynews/";  
	...
	public static HySpider getInstance() {
		return m_bzwspider;
	}
	/*
	 * （非 Javadoc）
...
//				newsdao.save(news);
	...
	public void init() {
		...
		try {
			wl = new edu.zju.cims201.spider.grabnews.uggdhy.SpiderSQLWorkload();
		} catch (Exception e) {
