...
import edu.zju.cims201.spider.grabnews.uggdrj.parser.ParserRJ;
import edu.zju.cims201.spider.grabnews.uggdrj.rule.RJUrlRule;
 ...
 * 
 */
public class RJSpider extends Thread implements ISpiderReportable {
	private static final RJSpider m_bzwspider = new RJSpider();
	...
	protected static String _hostUrl = "http://www.uggd.com/news/rjnews/";
...
//			); 
	...
	protected RJUrlRule rule = new RJUrlRule();
	protected ParserRJ parser = new ParserRJ();
	private static String spidername = "BzwSpider";

	private static String tablename = "workload";

	private static String urlpattern = "http://www.uggd.com/news/rjnews/\\d{4}-\\d{2}-\\d{2}/\\d{5}.html";

//	private static String defaulturl = "http://www.bzw.com.cn/article/class_other.asp?id=89&page=";
	private static String defaulturl = "http://www.uggd.com/news/rjnews/"; 
	...
	public static RJSpider getInstance() {
		return m_bzwspider;
	}
	/*
	 * （非 Javadoc）
...
//				newsdao.save(news);
	...
	public void init() {
		...
		try {
			wl = new edu.zju.cims201.spider.grabnews.uggdrd.SpiderSQLWorkload();
		} catch (Exception e) {
