...
import edu.zju.cims201.spider.grabnews.uggdzh.parser.ParserZH;
import edu.zju.cims201.spider.grabnews.uggdzh.rule.ZHUrlRule;
 ...
 * 
 */
public class ZHSpider extends Thread implements ISpiderReportable {
	private static final ZHSpider m_bzwspider = new ZHSpider();
	...
	protected static String _hostUrl = "http://expo.uggd.com/zhnews";
...
//			); 
	...
	protected ZHUrlRule rule = new ZHUrlRule();
	protected ParserZH parser = new ParserZH();
	private static String spidername = "BzwSpider";

	private static String tablename = "workload";

	private static String urlpattern = "http://expo.uggd.com/zhnews/\\d{4}-\\d{2}-\\d{2}/\\d{4,5}.html";

	private static String defaulturl = "http://expo.uggd.com/zhnews"; 
	...
	public static ZHSpider getInstance() {
		return m_bzwspider;
	}
	/*
	 * （非 Javadoc）
...
//				newsdao.save(news);
	...
	public void init() {
		...
		try {
			wl = new edu.zju.cims201.spider.grabnews.uggdzh.SpiderSQLWorkload();
		} catch (Exception e) {
