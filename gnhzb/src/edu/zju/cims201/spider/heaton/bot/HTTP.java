...
/**
 * This class implements an HTTP handler. The low-level details are left to a
 * derived class. Most likely you will be using the HTTPSocket class, which
 * derives from this one. This class handles these specific issues:
 *  + Cookies + The referrer tag + HTTP User Authentication + Automatic
 * Redirection + Parsing headers
 * 
...
public abstract class HTTP {
	...
	/**
	 * The raw header text that was recieved during the last send.
	 */
	...
	/**
	 * True if the HTTP class should automatically follow HTTP redirects.
	 */
	...
	/**
	 * The client headers, these are the headers that are sent to the server.
	 */
	protected AttributeList _clientHeaders = new AttributeList();

	/**
	 * The server headers, these are the the headers that the server sends back.
	 */
	...
	/**
	 * The HTTP response(i.e. OK). If NOT okay, this response is thrown as an
	 * exception.
	 */
	 ...
	 */
	protected int _timeout = 300000;

	/**
	 * The page previously on. Used to accurately represent the referrer header.
	 */
	protected String _referrer = null;

	/**
	 * The agent(or browser) that the HTTP class is reporting to the server.
	 */
	...
	/**
	 * The copy method is used to create a new copy of this HTTP handler. This
	 * method is implemented abstract and should be implemented by derived
	 * classes.
	 * 
	...
	/**
	 * This method actually sends the data. This is where derived classes
	 * implement their own low-level send.
	 * 
	 * @param url
	 *            The URL to send data to.
	 * @param post
	 *            Any data that is to be posted. If no data is posted, this
	 *            method does an HTTP GET.
	 * @exception java.net.UnknownHostException
	 *                Thrown if the host can not be located.
	...
	abstract protected void lowLevelSend(String url, String post, String pageEncode)
	throws java.net.UnknownHostException, java.io.IOException;
	/**
	 * This method is called to clear any cookies that this HTTP object might be
	 * holding.
	 */
	...
	/**
	 * This method is called to add any applicable cookies to the header.
	 */
	protected void addCookieHeader() {
		...
		log.trace("Send cookie: " + str);
		// Log.log(Log.LOG_LEVEL_TRACE,"Send cookie: " + str );
	}

	/**
	 * This method is called to add the user authorization headers to the HTTP
	 * request.
	 */
	...
	/**
	 * Send is the public interface to this class that actually sends an HTTP
	 * request.
	 * 
	 * @param url
	...
	public void send(String url, String post)
		...
		while (true) {
			...
			log.info("HTTP REDIRECT to " + u.toString());
	...
	public void send(String url, String post, String pageEncode)
			throws java.net.UnknownHostException, java.io.IOException {
		int rtn; // the return value
		...
		while (true) {
			if (_useCookies)
				addCookieHeader();
			addAuthHeader();
			lowLevelSend(_url, post, pageEncode);
	...
	/**
	 * This method is called to get the body data that was returned by this
	 * request.
	 * 
	...
	/**
	 * Get the URL that was ultimately requested. You might get a different URL
	 * than was requested if auto redirection is enabled.
	 */
	...
	/**
	 * Called to set the internal URL that is kept for the last request.
	 * 
	 * @param u
	 *            The new URL.
	 */
	...
	/**
	 * Used to determine if the HTTP class should automatically resolve any HTTP
	 * redirects.
	 * 
	 * @param b
	 *            True to resolve redirects, false otherwise.
	 */
	...
	/**
	 * This method is called internally to parse any cookies and add them to the
	 * cookie store.
	 */
			...
			if (a.getName().equalsIgnoreCase("set-cookie")) {
				...
				if (_cookieStore.get(cookie.get(0).getName()) == null) {
					if ((cookie.get("expires") == null)
							|| (cookie.get("expires") != null)
							&& _usePermCookies)
						_cookieStore.add(cookie);
				}
				log.info("Got cookie: " + cookie.toString());
				// Log.log(Log.LOG_LEVEL_TRACE,"Got cookie: " +
				// cookie.toString() );
			}
		}
	}
	 ...
	 * 
	 * @param name
	 *            The name of the cookie being sought.
	 ...
	 * 
	 * @param session
	 *            True to use session cookies, false not to.
	 * @param perm
	 *            True to use permanent cookies, false not to.
	 */
	...
	/**
	 * This method allows you to determine if session cookies are being used.
	 * 
	...
	/**
	 * This method allows you to determine if permanent cookies are being used.
	 * 
	 * @return Returns true if permanent cookies are being used, false
	 *         otherwise.
	 */
				...
				if (name.length() == 0)
					...
					if (first) {
						...
						if (ptr != -1) {
							...
							log.trace("Sever Header:" + name + "=" + value);
							// Log.log(Log.LOG_LEVEL_TRACE,"Sever Header:" +
							// name + "=" + value);
						}
					}
				}
	...
	/**
	 * This method is called to set the amount of time that the HTTP class will
	 * wait for the requested resource.
	 * 
	 * @param i
	 *            The timeout in milliseconds.
	 */
	...
	/**
	 * This method will return the amount of time in milliseconds that the HTTP
	 * object will wait for a timeout.
	 */
	...
	/**
	 * This method will set the maximum body size that will be downloaded.
	 * 
	 * @param i
	 *            The maximum body size, or -1 for unlimted.
	 */
	...
	/**
	 * This method will return the maximum body size that will be downloaded.
	 * 
	...
	/**
	 * Returns the user agent being reported by this HTTP class. The user agent
	 * allows the web server to determine what software the web-browser is
	 * using.
	 * 
	 ...
	 * 
	 * @param a
	 *            The user agent string to be reported to servers.
	 */
	...
	/**
	 * This method is called to set the user id for HTTP user authentication.
	 * 
	 * @param u
	 *            The user id.
	 */
	...
	/**
	 * This method is used to set the user's password for HTTP user
	 * authentificaiton.
	 * 
	 * @param p
	 *            The password.
	...
	/**
	 * This method is used to return the user id that is being used for HTTP
	 * authentification.
	 * 
	 * @return The user id.
	 */
	...
	/**
	 * This method is used to get the password that is being used for user
	 * authentication.
	 * 
	 * @return The password.
	 */
