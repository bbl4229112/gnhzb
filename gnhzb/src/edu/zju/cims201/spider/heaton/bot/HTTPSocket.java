...
// Process client headers
      ...
      do {
        ...
        if ( a!=null ) {
          ...
          headers.append("\r\n");
          log.trace("Client Header:" + a.getName() + "=" + a.getValue());
...
// Send a blank line to signal end of HTTP headers
      writeString(out,"");
      if ( post!=null ) {
        log.trace("Socket Post(" + post.length() + " bytes):" + new String(post) );
...
// now parse the headers and get content length
      ...
      try {
        ...
        log.error("Bad value for content-length:",e);
      ...
      if ( contentLength!=0 ) {
        ...
        while ( (contentLength--)>0 ) {
          ...
          if ( l<0 )
            break;
          _body.append(new String(buffer,0,l,"UTF-8"));//数据编码 8859_1
          max--;
        ...
        do {
          ...
          if ( l<0 )
            break;
          _body.append(new String(buffer,0,l,"UTF-8"));
          max--;
          if ( max<0 && (_maxBodySize!=-1) )
            break;
        } while ( l!=0 );
      }
      log.debug("Socket Page Back:" + _body + "\r\n" );
...
// Cleanup
  ...
  synchronized public void lowLevelSend(String url,String post, String pageEncode)
  throws java.net.UnknownHostException, java.io.IOException
  {
    String command;// What kind of send POST or GET
    StringBuffer headers;// Used to hold outgoing client headers
...
// now parse the headers and get content length
      ...
      if ( contentLength!=0 ) {
        ...
        while ( (contentLength--)>0 ) {
          ...
          if ( l<0 )
            break;
          _body.append(new String(buffer,0,l,pageEncode));//数据编码 8859_1
          max--;
          if ( max<0 && (_maxBodySize!=-1) )
            break;
        }
        ...
        do {
          ...
          if ( l<0 )
            break;
          _body.append(new String(buffer,0,l,pageEncode));
          max--;
          if ( max<0 && (_maxBodySize!=-1) )
            break;
        } while ( l!=0 );
