 ...
 */
public class Graph extends CompositeTupleSet  implements Serializable{
    
    /**
	 * 
	 */
	private static final long serialVersionUID = -2371161157202267562L;
    ...
    = PrefuseConfig.get("data.graph.relation"); 
    public static final String DEFAULT_RELATION_KEY 
    = "relation"; 
    ...
    //hebi 添加关系key
    /** The relation node key field (for the Edge table) */
    ...
    public Graph(Table nodes, boolean directed) {
        this(nodes, directed, DEFAULT_NODE_KEY,
                DEFAULT_SOURCE_KEY, DEFAULT_TARGET_KEY,DEFAULT_RELATION_KEY);
    }
    ...
    public Graph(Table nodes, boolean directed, String nodeKey,
        ...
        //      //hebi 添加关系key
        edges.addColumn(relationkey, String.class, "");
     ...
     * table
     */
    public Graph(Table nodes, Table edges, boolean directed,String nodeKey,
            String sourceKey, String targetKey
      
    )
    {
        init(nodes, edges, directed, nodeKey, sourceKey, targetKey      	       //hebi 添加关系key
                , "relation"		
        );
    }
...
//    	System.out.println("relationKey="+relationKey);
//       	System.out.println(nodes.getColumnType(nodeKey));
//    	System.out.println(edges.getColumnType(sourceKey));
//    	System.out.println(edges.getColumnType(targetKey));
//    	System.out.println(edges.getColumnType(relationKey));
//        if ( (nodeKey!=null && 
...
//        }
    ...
    public void setTupleManagers(TupleManager ntm, TupleManager etm) {
        if ( !Node.class.isAssignableFrom(ntm.getTupleType()) ){
            System.out.println("The provided node " +
        ...
        if ( !Edge.class.isAssignableFrom(etm.getTupleType()) ){
            ...
            System.out.println("The provided edge " +
                "TupleManager must generate tuples that implement " +
                "the Edge interface.");    
    ...
    public void setEdgeTable(Table edges) {
        ...
        m_links.clear();
        //hebi修改
//        System.out.println("m_nkey="+m_nkey);
//        System.out.println("m_skey="+m_skey);
//        System.out.println("m_tkey="+m_tkey);
//        System.out.println("m_rkey="+m_rkey);
        init(getNodeTable(), edges, m_directed, m_nkey, m_skey, m_tkey,m_rkey);
    }
    ...
    //hebi添加
    public String getEdgeRelationField() {
        return m_rkey;
    }
    /**
    ...
    public int getRelationNode(int edge) {
        return getNodeIndex(getEdgeTable().getLong(edge, m_rkey));
    }
    /**
 ...
 //hebi添加
    public Node getRelationNode(Edge e) {
        edgeCheck(e, true);
        return getNode(getRelationNode(e.getRow()));
    }
    ...
    public Tree getSpanningTree(Node root) {
        ...
        if ( m_spanning == null ) {
            m_spanning = new SpanningTree(this, root);
           	System.out.println("m_spanning == null ");
        } else if ( m_spanning.getRoot() != root ) {
        	System.out.println("m_spanning.getRoot() != root");
            m_spanning.buildSpanningTree(root);
        }
        return m_spanning;
    }
    public Tree getSpanningTree2(Node root) {
        nodeCheck(root, true);
        if ( m_spanning == null ) {
            m_spanning = new SpanningTree(this, root);
           	System.out.println("m_spanning == null ");
     ...
     */
    protected class Listener implements TableListener, ColumnListener,Serializable {

        /**
		 * 
		 */
		private static final long serialVersionUID = 5433135669790647223L;
