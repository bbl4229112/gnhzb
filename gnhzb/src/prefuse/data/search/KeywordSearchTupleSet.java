...
public class KeywordSearchTupleSet extends SearchTupleSet {
    ...
    protected Document getDocument(int id, String text) {
        ...
        if(m_storeTermVectors)
        d.add(new Field(LuceneSearcher.FIELD, text, Field.Store.YES,Field.Index.ANALYZED));
        else
        	d.add(new Field(LuceneSearcher.FIELD, text, Field.Store.NO,Field.Index.ANALYZED));	      
        d.add(new Field(LuceneSearcher.ID, String.valueOf(id),Field.Store.YES,Field.Index.ANALYZED));
        return d;
    }
