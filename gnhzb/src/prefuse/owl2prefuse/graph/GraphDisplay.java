...
public class GraphDisplay extends Display {
	 ...
	 */
	public GraphDisplay(Graph p_graph, boolean p_distancefilter, int i, int t,
			String direction) {
	...
	private ActionList getDrawActions(boolean p_distancefilter) {
		...
		// 设置展示到焦点的距离 默认是3
		if (p_distancefilter) {
			m_filter = new GraphDistanceFilter(Constants.GRAPH, 6);
			draw.add(m_filter);
	...
	private ActionList getLayoutActions() {
		...
		// layout.add(new AxisLayout(Constants.GRAPH));
