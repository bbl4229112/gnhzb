...
public class OWLGraphConverter extends Converter {
	...
	private Hashtable<String, Edge> m_edgess;
	...
	private void init(boolean p_directed, String searchname) {
		...
		m_edgess= new Hashtable<String, Edge>();
...
//		}
				...
				if (property.getObject().isResource()) {
					...
					if (!m_uselessType.contains(targetURI)
						...
						if (!m_nodesadded.containsKey(sourceURI)) {
							...
							// System.out.println("sourcelist.add("+sourceName+");");
	...
	private void listsub() {
		...
		for (int u = 0; u < sourcelist.size(); u++) {
			...
			// System.out.println(" sourcelist.get(u).toString()="+
...
//			}
			...
			if (m_nodes.containsKey(strEdge[0] + "的" + strEdge[1])) {
				edge=m_edgess.get(strEdge[0] + "的" + strEdge[1]);
				
				relation = m_graph.getTargetNode(edge);
				// relation= g.get(strEdge[0]+"的"+strEdge[1]);
				...
				indproperty += "<Strong><font color="
						...
						+ "> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" + strEdge[2]
						+ "</font></Strong><br>";
		//		System.out.println("indproperty=" + indproperty);
		...
		//		System.out.println("m_nodes.put(" + strEdge[0] + "的"+ strEdge[1] + ", relation)");
			}
           
			edge = m_graph.addEdge(source, relation);
            if(!m_edgess.containsKey(strEdge[0] + "的" + strEdge[1]))
            	m_edgess.put(strEdge[0] + "的" + strEdge[1], edge);
