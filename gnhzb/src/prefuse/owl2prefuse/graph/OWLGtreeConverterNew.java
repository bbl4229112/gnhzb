...
public class OWLGtreeConverterNew {
	...
	public OWLGtreeConverterNew(String owlPath, String searchname) {
...
//创建子树的根节点 rootnode 是从大树上找到的子树的原始根节点，需要重新创建赋值，本方法就是完成这个工作
	private void creatroot(Node rootNode) {
		...
		creatgraph(rootNode,newNode);

	};
//依次遍历构建子节点	
//rootNode是大树原始节点，newNode是当时对应的新树的节点	
	private void creatgraph(Node oldNode, Node newNode) {
		if(m_graph.getEdgeTable().getRowCount()<3000){
...
//如果指出（outEdges）边不为空则遍历找到其target节点
		...
		while (outEdgesIT.hasNext()) {
		    ...
		    //target子节点在新建的树上还没有添加
				if(subnode.getString("URI").equals("运载火箭的Z"))
					System.out.println("外面"+subnode.getString("URI"));	
				if (!existsubnodes.containsKey(subnode.getString("URI"))) {
					if(subnode.getString("URI").equals("运载火箭的Z"))
					System.out.println("第一次创建"+subnode.getString("URI"));			
				//在新树上添加对应节点	
			    ...
			    //继续构建下面的分支
					creatgraph(subnode,newsubNode);
				}
				//target子节点在新建的树上还已经添加，处理重复问题，这里需要区别术语节点和关系节点
				else
				{   
//					System.out.println("重复的节点uri"+subnode.getString("URI")); 
//				    System.out.println("type"+subnode.getString("type"));
                 //得到当前新树上的target节点
				 ...
				 // 如果目标节点是关系节点，这里通过type来判断，type位null就是关系节点	
					if (subnode.getString("type") == null) {
							// 继续构建新树，它不会造成死循环
							creatgraph(subnode, newsubNode);
							System.out.println(subnode.getString("name"));
	...
	private void wrapNode(Node oldNode, Node newNode,String type) {
	    ...
	    if(type.equals("root"))
		newNode.setString("image", "c://onto/root.gif");
	    else if(type.equals("end")){
	    newNode.setString("image", "c://onto/3.gif");
	    newNode.setString("name", oldNode.getString("name")+"*");
	...
	{
		OWLGtreeConverterNew cvt=new OWLGtreeConverterNew("prefuse/owl2prefuse/all.owl", "运载火箭");
		cvt.getGraph();
	}
}
