...
public class EdgeRenderer extends AbstractShapeRenderer {
    ...
    //何必修改该  protected Point2D m_tmpPoints[]  = new Point2D[2];
    protected Point2D m_tmpPoints[]  = new Point2D[3];
    ...
    public EdgeRenderer() {
        ...
        m_tmpPoints[2]  = new Point2D.Float();
    ...
    protected Shape getRawShape(VisualItem item) {
      ...
      //hebi添加 用于记录edge的关系名称 暂时用targetid来代替
   //     VisualItem item3 = edge.getedgerelationItem();
        relation=edge.getString("relation");
    //    System.out.println("here   relation="+relation);
        int type = m_edgeType;
        //获得第一个坐标
        ...
        getAlignedPoint(m_tmpPoints[1], item2.getBounds(),
                        m_xAlign2, m_yAlign2);
        //hebi添加 获得第三个重点坐标？
        getAlignedPointm(m_tmpPoints[2],m_tmpPoints[0], m_tmpPoints[1]);
       //System.out.println("获得的坐标点分别是x="+m_tmpPoints[2].getX()+"    y="+m_tmpPoints[2].getY());
      //获得先的宽度？
    ...
    public void render(Graphics2D g, VisualItem item) {
        ...
        if ( m_curArrow != null ) {
            ...
            g.fill(m_curArrow);
          //  System.out.println("zhaozhongddian");
            g.drawString(relation, (int)m_tmpPoints[2].getX(), (int)m_tmpPoints[2].getY());
        }
    }
    ...
    //hebi添加 给出中点的位置
    protected static void getAlignedPointm(Point2D p, Point2D p1, Point2D p2 ) {
        double x = p1.getX(), y = p1.getY();
        double x2 = p2.getX(), y2 = p2.getY();
            x = (x+x2)/2;
            y = (y+y2)/2;
     
        //    System.out.println("获得的坐标点分别是x="+x+"    y="+y);
        p.setLocation(x,y);
    }
    /**
